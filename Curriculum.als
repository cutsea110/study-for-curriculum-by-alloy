...
enum 曜時 { 日１, 日２, 日３, 日４, 日５, 日６, 日７, 日８,
...
fact 隔週時間割でなければ隔週相手はない{
	all t: this/時間割 |
		t.週間隔区分 = 毎週 => no t.隔週相手
}

fact 隔週時間割は相手が存在するなら対になっている{
	all t: this/時間割 {
		t.週間隔区分 in (隔週1 + 隔週2) =>
			let t' = t.隔週相手 {
				some t' => t = t'.隔週相手 and t.週間隔区分 != t'.週間隔区分
			}
	}
}
...
assert 隔週相手は対称的な関係になっている{
	隔週相手 in ~隔週相手
}
check 隔週相手は対称的な関係になっている
...
run 隔週時間割の相手時間割を定義できる{
	some t: this/時間割 |
		t.週間隔区分 in (隔週1 + 隔週2) and some t.隔週相手
}
