...
assert 任意の子時間割には代表時間割が存在する{
	all c: 子時間割 | some r: 代表時間割 |
		c.^代表 = r
}
check 任意の子時間割には代表時間割が存在する

assert 任意の異なる代表時間割の子時間割は排他的である{
	all disj r,r': 代表時間割 |
		no (r.^(~代表) & r'.^(~代表))
}
check 任意の異なる代表時間割の子時間割は排他的である
...
run 代表時間割で異なる科目の時間割をまとめられる{
	some disj j,j': this/時間割 |
		j.授業.科目 != j'.授業.科目 and j.^代表 = j'.^代表
}
run 代表時間割で異なる科目の時間割の履修を取得できる{
}
run 代表時間割で異なる科目の時間割の出欠情報を取得できる{
}
run 代表時間割で異なる科目の時間割の試験成績を取得できる{
}
run 代表時間割で異なる科目の時間割の成績を取得できる{
}
--------------------------
